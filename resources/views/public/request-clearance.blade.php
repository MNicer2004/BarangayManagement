...
<style>
  ...
  /* Select: light bg + dark text for readability */
  ...
  .form-shell .form-select:focus{
    border-color: #4A7FA7;
    box-shadow: 0 0 0 .15rem rgba(74,127,167,.25);
    background: #F6FAFD;
    color: #0A1832;
  }
  ...
  /* Submit button hover: blue */
  .btn-submit{
    background:#F6FAFD; color:#0A1832; border-color:#F6FAFD;
    font-weight:700;
  }
  .btn-submit:hover{
    background:#1A3D63; border-color:#1A3D63; color:#fff;
  }
</style>
...
<section class="py-4 py-md-5">
  <div class="container" style="max-width:1200px">
    <div class="row g-4 g-xl-5">
      <!-- LEFT: Logo + info + requirements + notes -->
      <div class="col-12 col-lg-4">
        <div class="card-glass side-card h-100">
          ...
          <div class="px-3 px-md-4 pt-3">
            ...
            <div class="item-check">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#4A7FA7" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
              <span>Proof of residency (Barangay ID/Voter’s record/utility bill under your name)</span>
            </div>
            <div class="item-check">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#4A7FA7" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
              <span>1×1 or 2×2 photo (recent), if required</span>
            </div>
          </div>
          ...
          <div class="px-3 px-md-4 py-3">
            ...
            <div class="item-check">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#4A7FA7" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
              <span>Collect your document at the <strong>Barangay Hall</strong>. Bring the <strong>original ID</strong> you used.</span>
            </div>
            <div class="item-check">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#4A7FA7" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
              <span>For <strong>GCash</strong> payments, show the <strong>GCash reference number</strong> at pickup.</span>
            </div>
            <div class="item-check">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#4A7FA7" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
              <span>Expected processing time: <strong>1–2 working days</strong> (subject to verification).</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="col-12 col-lg-8">
        <div class="card-glass form-shell">
          <div class="border-bottom border-white border-opacity-25 px-3 px-md-4 py-3">
            <h2 class="h5 m-0 fw-bold text-uppercase">Barangay Clearance</h2>
          </div>

          <form id="clearanceForm" class="p-3 p-md-4">
            <!-- Contact block -->
            <div class="row g-3">
              <div class="col-12">
            ...
            <div class="mt-3">
              ...
              <textarea id="purpose" class="form-control" rows="3" placeholder="Enter purpose (e.g., Job application, School requirement)" required></textarea>
            </div>
            ...
            <div class="mt-3">
              <div class="form-check">
                ...
                <label class="form-check-label" for="agreePrivacy">
                  I agree to the collection and processing of my personal data for the purpose of issuing this clearance.
                </label>
              </div>
              <div class="form-check mt-2">
                <input class="form-check-input" type="checkbox" id="agreeTruth" required>
                <label class="form-check-label" for="agreeTruth">
    ...
    <div class="modal-content" style="background:#0A1832;color:#F6FAFD;border:1px solid rgba(255,255,255,.2)">
      ...
      <div class="modal-body pt-0">
        ...
        <div class="d-flex align-items-center gap-2 mb-3">
          <code id="previewCode" class="fs-5 fw-bold">BCLR-0000-XXXXX</code>
...
<script>
  // Helpers: min date, GCash ref toggle, simple "success" flow with demo tracking code
  document.addEventListener('DOMContentLoaded', () => {
    ...
    if (pickup) pickup.min = iso;

    const method = document.getElementById('paymentMethod');
    const refWrap = document.getElementById('gcashRefWrap');
    const ref = document.getElementById('gcashRef');
    const toggleRef = () => {
      const show = method && method.value === 'GCash';
      refWrap.style.display = show ? '' : 'none';
      if (ref) { ref.required = show; if(!show) ref.value=''; }
    };
    if (method) { method.addEventListener('change', toggleRef); toggleRef(); }

    // Demo submit → show modal with generated code
    ...
    form.addEventListener('submit', (e) => {
      ...
      const genCode = () => {
        ...
        return `BCLR-${new Date().getFullYear()}-${rand}`;
      };
